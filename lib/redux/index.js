 ...
 * @param signal Optional `AbortSignal`, same caveat as `fetchPostJson`'s -- added for
 * `requestReductionPath` (#148), the first GET helper in this file that needs one. Every
 * other caller leaves this `undefined`, which is exactly what a bare `fetch(url)` used to
 * do, so this is not a behavior change for them.
 * @returns the parsed JSON body of the response.
 * @throws if the request fails (network error) or the response is not `ok`.
 */
async function fetchJson(url, failMsg, signal) {
...
/**
 * @param url Base URL of the Redux API.
 * @returns every `ComplexityClass` member name, sorted, including
 * "Unclassified" — callers must filter that out themselves, same convention
 * `data/taxonomy.js`'s translation maps already use.
 * @throws if the request fails.
 */
export function requestComplexityClasses(url) {
  return cachedRequest(`${url}|complexityClasses`, () =>
    fetchJson(`${url}Navigation/ComplexityClasses`, () => "COMPLEXITY CLASSES REQUEST FAILED"),
  );
}

/**
 * @param url Base URL of the Redux API.
 * @returns every `ProblemType` member name, sorted, including "Unclassified"
 * — callers must filter that out themselves.
 * @throws if the request fails.
 */
export function requestProblemTypes(url) {
  return cachedRequest(`${url}|problemTypes`, () =>
    fetchJson(`${url}Navigation/ProblemTypes`, () => "PROBLEM TYPES REQUEST FAILED"),
  );
}

/**
 * @param url Base URL of the Redux API.
 * @returns every `ReductionType` member name, sorted, including
 * "Unclassified" — callers must filter that out themselves.
 * @throws if the request fails.
 */
export function requestReductionTypes(url) {
  return cachedRequest(`${url}|reductionTypes`, () =>
    fetchJson(`${url}Navigation/ReductionTypes`, () => "REDUCTION TYPES REQUEST FAILED"),
  );
}

/**
 * @param url Base URL of the Redux API.
 * @returns every `ReductionCost` member name, sorted, including
 * "Unclassified" — callers must filter that out themselves.
 * @throws if the request fails.
 */
export function requestReductionCosts(url) {
  return cachedRequest(`${url}|reductionCosts`, () =>
    fetchJson(`${url}Navigation/ReductionCosts`, () => "REDUCTION COSTS REQUEST FAILED"),
  );
}

/**
 * @param url Base URL of the Redux API.
 * @returns every `SolverType` member name, sorted, including "Unclassified"
 * — callers must filter that out themselves.
 * @throws if the request fails.
 */
export function requestSolverTypes(url) {
  return cachedRequest(`${url}|solverTypes`, () =>
    fetchJson(`${url}Navigation/SolverTypes`, () => "SOLVER TYPES REQUEST FAILED"),
  );
}

/**
 * @param url Base URL of the Redux API.
 * @returns every `SolverComplexityBucket` member name, sorted, including
 * "Unclassified" — callers must filter that out themselves.
 * @throws if the request fails.
 */
export function requestSolverComplexityBuckets(url) {
  return cachedRequest(`${url}|solverComplexityBuckets`, () =>
    fetchJson(
      `${url}Navigation/SolverComplexityBuckets`,
      () => "SOLVER COMPLEXITY BUCKETS REQUEST FAILED",
    ),
  );
}
...
/**
 * Finds the cheapest reduction chain between two problems.
 *
 * #148: the reduction graph page's source/target picker calls this once both are picked.
 * The backend runs Dijkstra over `ReductionCost` (Linear/Quadratic/Cubic/
 * HigherPolynomial/Unclassified) as edge weight -- this is a real "cheapest chain"
 * lookup, not the plain unweighted BFS `hooks/useCatalogFilters.js`'s
 * reachableOneHop/reachableAnyHops already do client-side for the reachability filter
 * (T59/#134). `found: false` (with empty `nodes`/`hops`) is how the backend reports "no
 * path", including when `source` equals `target` -- that is a normal answer, not a
 * request failure, so callers should branch on `result.found` rather than expecting this
 * to reject for an unreachable pair.
 *
 * Deliberately not run through `cachedRequest` like the other GET helpers here: source
 * and target change on nearly every call (a different pair almost every time a visitor
 * uses the picker), so a URL-keyed cache would only ever produce single-use entries --
 * no repeat-call benefit, just unbounded growth in `requestCache` for a page session
 * where someone tries several pairs. Takes an `AbortSignal` for the same reason
 * `requestSolvedInstance` does: so picking a new pair before the previous lookup answers
 * cancels it instead of leaving a stale response racing the new one.
 *
 * @param {string} url Base URL of the Redux API.
 * @param {string} source Source problem name (matched case-insensitively by the backend).
 * @param {string} target Target problem name (matched case-insensitively by the backend).
 * @param {AbortSignal} [signal] Optional, same caveat as `requestSolvedInstance`.
 * @returns {Promise<{found: boolean, nodes: string[], hops: Array<{from: string, to: string, className: string, cost: string}>}>}
 * @throws {ReduxRequestError} if the request fails.
 */
export function requestReductionPath(url, source, target, signal) {
  return fetchJson(
    `${url}Navigation/Reductions/path?source=${encodeURIComponent(source)}&target=${encodeURIComponent(target)}`,
    () => `REDUCTION PATH REQUEST FAILED (${source} -> ${target})`,
    signal,
  );
}

/**
 * Added for the About Us page, ported from Redux_GUI's `requestContributorDirectory`.
 *
 * @param url Base URL of the Redux API.
 * @returns an array of `{name, githubUsername}` for every contributor listed in the
 * backend's `contributorInfo.json`. `githubUsername` is an empty string for a
 * contributor who has not added theirs yet.
 * @throws if the request fails.
 */
...
/**
 * Added for the About Us page, ported from Redux_GUI's `requestContributorProfile`.
 *
 * @param url Base URL of the Redux API.
 * @param contributorName A contributor's full display name, e.g. "Andrija Sevaljevic",
 * exactly as `requestContributorDirectory` lists it.
 * @returns one contributor's full profile: personal info, GitHub commit/PR/review
 * stats per repo, and every problem, solver, reduction, verifier and visualization
...
export function requestContributorProfile(url, contributorName) {
  return cachedRequest(`${url}|contributorProfile|${contributorName}`, () =>
    fetchJson(
      ...
      () => `${contributorName} CONTRIBUTOR PROFILE REQUEST FAILED`,
    ),
  );
}

...
# Chromium for the integration tests (see TESTING.md, once a suite exists — this currently sits
# unused since there is no tests/e2e/ yet, but installs cleanly so it's ready when one lands).
# Guarded like mise above: this also runs in CI via devcontainers/ci, so a download failure must
# not fail container creation or redden a PR.
#
